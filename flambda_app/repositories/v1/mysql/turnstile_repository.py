...
class TurnstileRepository(AbstractRepository):
    BASE_TABLE = 'turnstile'
    BASE_SCHEMA = 'store'
    BASE_TABLE_ALIAS = 't'
    PK = 'id'
    UUID_KEY = 'uuid'
    ...
    def create(self, turnstile: TurnstileVO):
        keys = list(turnstile.to_dict().keys())
        keys.remove(self.PK)
        keys_str = ",".join(keys)
        values_count = len(turnstile.to_dict().values()) - 1
        values_str = ",".join(['%s' for _ in range(values_count)])

        sql = "INSERT INTO {} ({}) VALUES ({})".format(self.BASE_TABLE, keys_str, values_str)

        turnstile_dict = turnstile.to_dict()
        del turnstile_dict[self.PK]
        values = tuple(turnstile_dict.values())

        try:
            created = self._execute(sql, values)
            turnstile.id = self.connection.insert_id()
    ...
    def update(self, turnstile: TurnstileVO, value, key=None):
        key_type = '%s'
        if key is None:
            key = self.PK
        ...
        values = []
        update_data = []
        for k, v in turnstile.to_dict().items():
            if k in keys:
        ...
        turnstile_dict = turnstile.to_dict()
        del turnstile_dict[self.PK]
        del turnstile_dict[self.UUID_KEY]
        values.append(value)
    ...
    def get(self, value, key=None, where: dict = None, fields: list = None):
        ...
        try:
            ...
            return TurnstileVO(item) if item else None
        except Exception as err:
            self.logger.error(err)
        finally:
            self._close()
